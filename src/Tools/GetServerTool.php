...
#[Description('Get the details of a specific Laravel Forge server.')]
class GetServerTool extends ForgeTool
{
    protected string $name = 'get_server';

    protected function run(Request $request, Forge $forge, string $slug): Response
    {
        return Response::json($forge->server($slug, (int) $request->get('serverId')));
    }
